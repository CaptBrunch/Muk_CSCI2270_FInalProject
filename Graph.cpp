...
void Graph::addVertex(string n)//insert vertecies
{
    bool found = false;
    for(int i = 0; i < vertices.size(); i++){
...
vertex * Graph::findVertex(std::string name) //find vert
{
    for(int i = 0; i < vertices.size(); i++)
    {
...
void Graph::shortestDistance(std::string startingCity,std::string endingCity)//find shortest distance
{
    ...
    vertex * endC = findVertex(startingCity);

    if (start == NULL || endC == NULL)
    {
        cout << "One or more cities doesn't exist,or it is not in our Database" << endl;
        return;
    }
    if (endC->district != start->district)
    {
        cout << "You can't get there by car" << endl;
        return;
    }
    if (endC->district == -1 || start->district == -1)
    {
        cout << "Please identify the districts before checking distances" << endl;
        return;
    }
...
void Graph::shortestDistanceRoundTrip(std::string startingCity)
{
    ...
    // Contains all of the possible path solutions
...
List::List()//constructor for link list
{
    head = NULL;
    curr = NULL;
}

void List::createLL(string name,string landM,int cost)
{
   nodePtr n = new city;
   n->name = name;
   n->landmark.name = landM;
   n->landmark.cost = cost;
}
...
void List::Budget(string city,int bu) //funtcion that based on the city and your budget outputs what you can afford
{
    ...
    {
        cout << "Maybe you can find out somewhere else to go to fit your budget" << endl;
    }
